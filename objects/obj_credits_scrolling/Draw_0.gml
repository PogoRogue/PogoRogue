...
if global.use_controller = true {
	scr_Draw_Text_Outlined(xx,416,"按         滚动",c_white);
	draw_sprite(scr_Gamepad_Get_Button_Sprite(global.gamepad_array[16][0]),0,xx-44,416);
}else {
	...
	if !scr_In_Array(global.mouse_button_array,keyboard_array_value) {
		if is_string(keyboard_text) {
			scr_Draw_Text_Outlined(xx,416,"按 " + string(keyboard_text) + " 滚动",c_white);
		}else {
			scr_Draw_Text_Outlined(xx,416,"按         滚动",c_white);
			draw_sprite(keyboard_text,0,xx-44,416);
		}
	}else {
		var mouse_sprite = scr_Mouse_Get_Button_Sprite(keyboard_array_value);
		scr_Draw_Text_Outlined(xx,416,"按         滚动",c_white);
		draw_sprite(mouse_sprite,0,xx-44,416);
	}
}

...
if (colliding) {
	...
	if global.use_controller = true {
		scr_Draw_Text_Outlined(x,bbox_top+16,"Press         To Choose",c_white);
		scr_Draw_Text_Outlined(x,bbox_top+32,"Active Item (For Testing)",c_white);
		draw_sprite(scr_Gamepad_Get_Button_Sprite(global.gamepad_array[6][0]),0,x-12+temp_x_offset,bbox_top+16);
	}else {
		...
		if !scr_In_Array(global.mouse_button_array,keyboard_array_value) {
			if is_string(keyboard_text) {
				scr_Draw_Text_Outlined(x,bbox_top+16,"Press " + string(keyboard_text) + " To Choose",c_white);
				scr_Draw_Text_Outlined(x,bbox_top+32,"Active Item (For Testing)",c_white);
			}else {
				scr_Draw_Text_Outlined(x,bbox_top+16,"Press         To Choose",c_white);
				scr_Draw_Text_Outlined(x,bbox_top+32,"Active Item (For Testing)",c_white);
				draw_sprite(keyboard_text,0,x-12+temp_x_offset,bbox_top+16);
			}
		}else {

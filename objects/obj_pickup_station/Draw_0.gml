...
if (colliding) {
	...
	if global.use_controller = true {
		scr_Draw_Text_Outlined(x,bbox_top+36,"按         选择技能",c_white);
		//scr_Draw_Text_Outlined(x,bbox_top+48,"Active Item",c_white);
		...
		if !scr_In_Array(global.mouse_button_array,keyboard_array_value) {
			if is_string(keyboard_text) {
				scr_Draw_Text_Outlined(x,bbox_top+36,"按 " + string(keyboard_text) + " 选择技能",c_white);
				//scr_Draw_Text_Outlined(x,bbox_top+48,"Active Item",c_white);
			}else {
				scr_Draw_Text_Outlined(x,bbox_top+36,"按         选择技能",c_white);
				//scr_Draw_Text_Outlined(x,bbox_top+48,"Active Item",c_white);

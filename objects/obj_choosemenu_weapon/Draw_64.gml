...
scr_Draw_Text_Outlined(center_x,center_y-56,"Choose your starting weapon:", c_white); //center_y - 140
...
for (i = 0; i < array_length(weapons_array); i++) {
	...
	var yy = (center_y+8) + floor(i / select_x_max) * y_gap; //156 + floor
...
for (i = 0; i < array_length(all_weapons); i++) {
	var xx = (center_x - ((x_gap*select_x_max-x_gap)/2)) + (i % select_x_max) * x_gap;
	var yy = 295 + floor(i / select_x_max) * y_gap; //226
	if (i = select-1) {

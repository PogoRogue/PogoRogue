...
if (colliding) {
	...
	if global.use_controller = true {
		scr_Draw_Text_Outlined(x,bbox_top+32,"Press         To",c_white);
		scr_Draw_Text_Outlined(x,bbox_top+48,"Input A Seed",c_white);
		draw_sprite(scr_Gamepad_Get_Button_Sprite(global.gamepad_array[6][0]),0,x+8+temp_x_offset,bbox_top+32);
	}else {
		...
		if !scr_In_Array(global.mouse_button_array,keyboard_array_value) {
			if is_string(keyboard_text) {
				scr_Draw_Text_Outlined(x,bbox_top+32,"Press " + string(keyboard_text) + " To",c_white);
				scr_Draw_Text_Outlined(x,bbox_top+48,"Input A Seed",c_white);
			}else {
				scr_Draw_Text_Outlined(x,bbox_top+32,"Press         To",c_white);
				scr_Draw_Text_Outlined(x,bbox_top+48,"Input A Seed",c_white);
				draw_sprite(keyboard_text,0,x+8+temp_x_offset,bbox_top+32);
			}
		}else {
			var mouse_sprite = scr_Mouse_Get_Button_Sprite(keyboard_array_value);
			scr_Draw_Text_Outlined(x,bbox_top+32,"Press         To",c_white);
			scr_Draw_Text_Outlined(x,bbox_top+48,"Input A Seed",c_white);
			draw_sprite(mouse_sprite,0,x+8+temp_x_offset,bbox_top+32);
		}
	}
}else {
	image_index = 0;	

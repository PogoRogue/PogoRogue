...
if (colliding) and being_used = false and sprite_index != spr_pogosmith_workbench_animation {
	...
	scr_Draw_Text_Outlined(x,bbox_top+48,"升级武器",c_white); //Select Upgrade
...
if being_used = true {
	...
	draw_set_font(global.lana);
	scr_Draw_Text_Outlined(x,bbox_top+56,"离开",c_white);
	...
	if !(select_x = 0 and gun_1_bought = true) and !(select_x = 1 and gun_2_bought = true) and !(select_x = 2 and gun_3_bought = true)  {
		scr_Draw_Input_UI(arrow_x,bbox_top+4,18,0,global.lana,fa_center,fa_middle);
	}
	draw_set_font(global.lana);
